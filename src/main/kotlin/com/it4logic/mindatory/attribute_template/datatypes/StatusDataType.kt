...
/**
 * Attribute Template Status Data Type management class
 */
class StatusDataType : AttributeTemplateDataType {
	data class StatusItem (var id: Long, var value: String, var color: String)

	override val identifier: UUID
		get() = DataTypeUUID.Status.toUUID()

	override val name: String
		get() = DataTypeName.Status
	...
	override fun validate(
		...
		try {
			if (property.value.isNotBlank()) {
				data = ObjectMapper().readValue<List<StatusItem>>(property.value)
			}
		} catch (ex: Exception) {
			when (ex) {
				is IOException,
