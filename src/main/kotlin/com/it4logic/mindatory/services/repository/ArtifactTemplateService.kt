...
class ArtifactTemplateService : ApplicationBaseService<ArtifactTemplate>() {
	...
	fun getDesignVersion(id: Long, versionId: Long): ArtifactTemplateVersion {
		...
		throw ApplicationObjectNotFoundException(
			versionId,
			ArtifactTemplateVersion::class.java.simpleName.toLowerCase()
		)
	}

	fun createVersion(
		artifactTemplateId: Long,
	...
	fun updateVersion(
		...
		val result = artifactTemplateVersionRepository.findOneByIdAndArtifactTemplateId(
			artifactTemplateVersion.id,
			artifactTemplateId
		).orElseThrow {
