...
//    val enc = Encryptors.text("12345678", "12345678")
    val tokenObject = TokenObject(user.id, UUID.randomUUID().toString())
    val tokenDate = Date()
    val cal = Calendar.getInstance()
    cal.time = tokenDate
    cal.add(Calendar.DATE, 1)
    val tokenExpireDate = cal.time
  ...
  fun findToken(token: String): SecurityUserToken {
    return userTokenRepository.findOneByToken(token).orElseThrow { ApplicationObjectNotFoundException(token, type().simpleName.toLowerCase()) }
  }
